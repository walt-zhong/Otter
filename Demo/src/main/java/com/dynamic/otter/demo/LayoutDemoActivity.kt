...
class LayoutDemoActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val context = OTEngine.instance
            .init(this)
            .buildTemplate("layout-demo", "layout","templates/layout-demo/layout.xml")

        DataBinding.instance.buildTemplateData(
            AssetsUtils.parseAssetsToString(
            this,
            "templates/layout-demo/mock-data/layout.json"))
        val rootView = OTEngine.instance.createAndroidView(context)
        findViewById<LinearLayoutCompat>(R.id.layout_container).addView(rootView)
    }
}