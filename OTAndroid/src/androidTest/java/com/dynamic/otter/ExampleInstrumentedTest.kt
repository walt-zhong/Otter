...
class ExampleInstrumentedTest {
    ...
    fun useAppContext() {
        ...
        assertEquals("com.senseauto.sensedyne.test", appContext.packageName)
    }

    val instance = ExpressionWrapper.instance

    @Test
    fun testExpression(){
        val testData = JSONObject().apply {
            this["eight"] = 8F
            this["title"] = "Hello,SenseDyne"
        ...
        val result = instance.getResult("\$title", testData)
        println("zhongxj===>getValue==>$result")
    }

    @Test
    fun testStrIsNumber(){
        assertEquals(true,OTHelper.isNumeric("11"))
        assertEquals(false,OTHelper.isNumeric("11a"))
        assertEquals(false,OTHelper.isNumeric("a11b"))
        assertEquals(false,OTHelper.isNumeric("aaa"))
    }
    ...
    fun bad_case_1() {
        ...
        assertEquals(null, instance.getResult("\$data", null))
        assertEquals(null, instance.getResult("\$\$", null))

        assertEquals(true, instance.getResult("size(\$title)+1>1 ? true : false ", testData))
        assertEquals(
            2f,
        ...
        // java => null+'otter' = nullotter
        assertEquals("test", instance.getResult("\$data.stringEmpty + 'test'", testData))

        assertEquals(10f, instance.getResult("\$data+2", testData))

        // 应该抛异常
        assertEquals(10f, instance.getResult("\$data+2", testData))

        assertEquals(0L, instance.getResult("\$data ? 1 : 0", testData))
    }
    ...
    fun empty_test() {
        ...
        println("zhongxj===>getValue==>$result")
        repeat(10) {
            assertEquals(null, instance.getResult("\$data.stringEmpty", testData))
            assertEquals("", instance.getResult("''", testData))
            assertEquals(null, instance.getResult("", testData))
            assertEquals(null, instance.getResult("null", testData))
            assertEquals(0f, instance.getResult("size(null)", JSONObject()))
        }
    }
