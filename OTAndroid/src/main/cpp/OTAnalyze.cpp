 ...
 * 获取两个数值计算的结果
 */
OTATSNode OTAnalyze::doubleCalculate(OTATSNode left, OTATSNode right, string op) {
    OTATSNode result = OTATSNode(left.name, left.token, left.token);
    string name;
    if ((op == "?") || (op == ":") || (op == "?:") || (op == "==") || (op == "!=")) {
        //可以返回map和array
 ...
 * 获取单个数值计算的结果
 */
OTATSNode OTAnalyze::singleCalculate(OTATSNode left, string op) {
    OTATSNode result = OTATSNode(left.name, left.token, left.token);
 ...
 * 计算表达式结果函数
 */
long OTAnalyze::getValue(string expression, void *source) {
    char *input;
    int inputLength = expression.length();
    input = new char[inputLength + 2];
    vector<OTATSNode> array;
    vector<OTATSNode> arrayNum;
    array.reserve(128); // 预留128个空位
    ...
    while (p < strlen(input)) {
        if (input[p] == ' ') {
            ...
            OTATSNode token = scanner(synCode, p, input, this);
            ...
            if (token.token == "value" || token.token == "data") {
                ...
                if (res == 0) {
                    ...
                    OTValue *gxv = (OTValue *) res;
                    if (gxv->tag == OT_TAG_FLOAT) {
                        ...
                        tokenNum.token = "num";
                    } else if (gxv->tag == OT_TAG_LONG) {
                        tokenNum.name = to_string(gxv->intNum);
                        tokenNum.token = "long";
                    } else if (gxv->tag == OT_TAG_STRING) {
                        tokenNum.name = gxv->str;
                        tokenNum.token = "string";
                    } else if (gxv->tag == OT_TAG_BOOL) {
                        if (gxv->int32 == 1) {
                            tokenNum.name = "true";
                        } else {
                            tokenNum.name = "false";
                        ...
                        tokenNum.token = "bool";
                    } else if (gxv->tag == OT_TAG_ARRAY) {
                        tokenNum.name = to_string((long) (gxv->ptr));
                        tokenNum.token = "array";
                    } else if (gxv->tag == OT_TAG_MAP) {
                        tokenNum.name = to_string((long) (gxv->ptr));
                        tokenNum.token = "map";
                    } else if (gxv->tag == OT_TAG_NULL) {
                        tokenNum.name = "null";
                        tokenNum.token = "null";
                    } else {
                        tokenNum.name = "null";
                    ...
                    if (gxv->tag == OT_TAG_STRING && gxv->str != NULL) {
                        delete[] gxv->str;
                        gxv->str = NULL;
                    }
                    delete gxv;
    ...
    if (iter != cache.end()) {
        if (iter->second == "(0)") {
            OTATSNode res = arrayNum[0];
            OTValue *pointer;
            if (res.token == "string") {
                pointer = new OTValue(OT_TAG_STRING, res.name);
            } else if (res.token == "bool") {
                if (res.name == "true") {
                    pointer = new OTValue(OT_TAG_BOOL, 1);
                } else {
                    pointer = new OTValue(OT_TAG_BOOL, 0);
                }
            } else if (res.token == "num") {
                pointer = new OTValue(OT_TAG_FLOAT, (float) atof(res.name.c_str()));
            } else if (res.token == "long") {
                pointer = new OTValue(OT_TAG_LONG, (int64_t) atoll(res.name.c_str()));
            } else if (res.token == "map") {
                pointer = new OTValue(OT_TAG_MAP, (void *) atol(res.name.c_str()));
            } else if (res.token == "array") {
                pointer = new OTValue(OT_TAG_ARRAY, (void *) atol(res.name.c_str()));
            } else if (res.token == "null") {
                pointer = new OTValue(OT_TAG_NULL, 1);
            } else {
                pointer = new OTValue(OT_TAG_NULL, 1);
            }
            Res = (long) pointer;
 ...
 */
long
OTAnalyze::calculateCache(string cacheString, vector<OTATSNode> array, void *p_analyze,
                          void *source) {
    long *paramsStack;
    int paramsSize = 0;
    ...
    for (int i = 0; i < cacheString.length(); i++) {
        ...
        if (isNumber(cacheString[i])) {
            ...
            if (cacheString[i] == ',') {
                ...
                OTATSNode node = array[num1];
                if (node.token == "num") {
                    OTValue *par = new OTValue(OT_TAG_FLOAT, (float) atof(
                            node.name.c_str()));
                    paramsTempArray.push_back((long) par);
                } else if (node.token == "long") {
                    OTValue *par = new OTValue(OT_TAG_LONG, (int64_t) atoll(
                            node.name.c_str()));
                    paramsTempArray.push_back((long) par);
                } else if (node.token == "string") {
                    OTValue *par = new OTValue(OT_TAG_STRING,
                                               node.name.c_str());
                    paramsTempArray.push_back((long) par);
                } else if (node.token == "bool") {
                    if (node.name == "true") {
                        OTValue *par = new OTValue(OT_TAG_BOOL, 1);
                        paramsTempArray.push_back((long) par);
                    } else {
                        OTValue *par = new OTValue(OT_TAG_BOOL, 0);
                        paramsTempArray.push_back((long) par);
                    }
                } else if (node.token == "map") {
                    OTValue *par = new OTValue(OT_TAG_MAP, (void *) atol(
                            node.name.c_str()));
                    paramsTempArray.push_back((long) par);
                } else if (node.token == "array") {
                    OTValue *par = new OTValue(OT_TAG_ARRAY, (void *) atol(
                            node.name.c_str()));
                    paramsTempArray.push_back((long) par);
                } else if (node.token == "null") {
                    OTValue *par = new OTValue(OT_TAG_NULL, 1);
                    paramsTempArray.push_back((long) par);
                } else {
                    OTValue *par = new OTValue(OT_TAG_NULL, 1);
                    paramsTempArray.push_back((long) par);
                ...
                OTATSNode node = array[numFunction];
                //有参数
                ...
                if (funVal == 0) {
                    ...
                    OTValue *fun = (OTValue *) funVal;
                    //取出结果
                    if (fun->tag == OT_TAG_FLOAT) {
                        ...
                        node.token = "num";
                    } else if (fun->tag == OT_TAG_LONG) {
                        node.name = to_string(fun->intNum);
                        node.token = "long";
                    } else if (fun->tag == OT_TAG_BOOL) {
                        if (fun->int32 == 1) {
                            node.name = "true";
                            node.token = "bool";
                        } else {
                    ...
                    } else if (fun->tag == OT_TAG_STRING) {
                        node.name = fun->str;
                        node.token = "string";
                    } else if (fun->tag == OT_TAG_MAP) {
                        node.name = to_string((long) fun->ptr);
                        node.token = "map";
                    } else if (fun->tag == OT_TAG_ARRAY) {
                        node.name = to_string((long) fun->ptr);
                        node.token = "array";
                    } else if (fun->tag == OT_TAG_NULL) {
                        node.name = "null";
                        node.token = "null";
                    } else {
                        node.name = "null";
                    ...
                    array[numFunction] = node;
                    res = array[numFunction];
                    if (fun != NULL && fun->tag == OT_TAG_STRING && fun->str != NULL) {
                        delete[] fun->str;
                        fun->str = NULL;
                    }
                    if (fun != NULL) {
 ...
 * 语法分析并生成缓存表达式的过程
 */
long OTAnalyze::check(string s, vector<OTATSNode> array, void *p_analyze, void *source,
                      string expression) {
    string tree;
    if (array.size() == 1) {
        tree = "(0)";
    ...
    OTValue *pointer;
    OTAnalyze *analyze = (OTAnalyze *) p_analyze;
    ...
    int symbolSize = 0;
    OTATSNode *valueStack = new OTATSNode[sentence.size() + 2];
    int valueSize = 0;
    ...
    while (true) {
        ...
        if (new_status == "acc") {
            if (valueStack[0].token == "string") {
                pointer = new OTValue(OT_TAG_STRING, valueStack[0].name);
                ...
                pointer = new OTValue(OT_TAG_FLOAT, (float) atof(valueStack[0].name.c_str()));
            } else if (valueStack[0].token == "long") {
                pointer = new OTValue(OT_TAG_LONG, (int64_t) atoll(valueStack[0].name.c_str()));
            } else if (valueStack[0].token == "map") {
                pointer = new OTValue(OT_TAG_MAP, (void *) atol(valueStack[0].name.c_str()));
            } else if (valueStack[0].token == "array") {
                pointer = new OTValue(OT_TAG_ARRAY, (void *) atol(valueStack[0].name.c_str()));
                            ...
                            if (op == "(") {
                                ...
                                for (int i = valueSize - 1; i >= 0; i--) {
                                    ...
                                    if (valueStack[i].token == "id") {
                                        ...
                                        isFunction = false;
                                        paramsSize = 0;
                                        if (fun->tag == OT_TAG_STRING && fun->str != NULL) {
                                            delete[] fun->str;
                                            fun->str = NULL;
                                        }
                                        delete fun;
