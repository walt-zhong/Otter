...
//核心子程序
OTATSNode scanner(int &syn, int &p, char s[], void *p_analyze) {
    ...
    if (s[p] == '$') {
        if (s[p + 1] == '$') {
            ...
            //VALUE
            p++;
            while (isNum(s[p]) || isChar(s[p]) || s[p] == '[' || s[p] == ']' || s[p] == '.') { // $data.nodes[0].title
                token[count++] = s[p];
                p++;
            }
            token[count] = '\0'; //'\0'作为结束符 ,将单词分隔开
    ...
    if (sign == "bool" || sign == "op") {
        temp = OTATSNode(str1, str1, sign);
    } else {
        temp = OTATSNode(str1, sign, sign);
    }
    delete[]token;
    return temp;
}
